...
are deterministic and always pass on correct data.
...
def main() -> int:
    ...
    for bname in _BUCKET_NAMES:
        ...
        collapse_note = " ← collapse" if bname != "this_year" and end == buckets.get("this_year", {}).get("period_end") else ""
    ...
    for bname in _BUCKET_NAMES:
        ...
        print(f"  {bname:<16} {strt:<12} {end:<12}  {cnt:>14,}  {amt:>22,.2f}  "
              f"{'[  ?  ]':>10}  {'[  ?  ]':>10}")
    print()
    print("  If all buckets match Odoo UI (±1 EGP) → Stage 1 KPI 7 verified.")
    print("  Note: this_quarter and this_half are identical when today is in Q2/H1")
    print("        (Apr-Jun) — same bucket end date; identical UI results are correct.")
    print()
    print(_SEP)
