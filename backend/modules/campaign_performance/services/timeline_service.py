...
async def get_campaign_timeline(
    ...
    Args:
        ...
        trend_months: # of trailing Cairo months in the volume trend (fixed by the
            endpoint to DEFAULT_TREND_MONTHS).
    ...
    overrides), keyed by campaign_id + window + trend, so a test config never
    poisons the production cache — same pattern as the Level-1 service.

    Returns a dict matching schemas.CampaignTimeline.
    ...
    cache_key = _cache.make_key(
        f"{_CACHE_KEY_PREFIX}:{campaign_id}:w{window_months}:t{trend_months}"
    )
    if default_config:
        cached = _cache.get(cache_key)
        if cached is not None:
    ...
    # Date bound: first day of (current Cairo month minus max(window, trend) months),
    # expressed as a UTC datetime string for the Odoo domain (Odoo stores UTC-naive).
    ...
    try:
        ...
        # ── RPC c — this campaign's leads since the date bound (paged) ─────────
        leads = await _fetch_all(
            _client,
            _LEAD_MODEL,
            [(CAMPAIGN_FIELD, "=", campaign_id), ("create_date", ">=", bound_str)],
    ...
    # ── trend (last trend_months, oldest→newest, 0-filled) ────────────────────
    trend_months_list = [
    ...
    # ── periods (last window_months, oldest→newest, full funnel + maturation) ─
    period_months_list = [
        _month_str(_shift_months(current_month_start, -i))
    ...
    logger.info(
        ...
        f"window={window_months}mo trend={trend_months}mo | "
