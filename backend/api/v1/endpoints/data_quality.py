...
@router.get(
    ...
    response_model=PaginatedMissingContactResponse,
    summary="Opportunities missing a stage (paginated)",
)
@limiter.limit("30/minute")
async def crm_missing_stage(
    request: Request,
    page: int = Query(1, ge=1, description="Page number"),
    page_size: int = Query(50, ge=1, le=200, description="Results per page"),
    team_id: Optional[int] = Query(None, description="Filter by team ID"),
    ...
    if sort_field not in _VALID_SORT_FIELDS:
        sort = "create_date desc"
    return await service.missing_stage_response(
        page=page,
        page_size=page_size,
        team_id=team_id,
        salesperson_id=salesperson_id,
...
@router.get(
    ...
    response_model=PaginatedMissingContactResponse,
    summary="Opportunities missing a salesperson (paginated)",
)
@limiter.limit("30/minute")
async def crm_missing_salesperson(
    request: Request,
    page: int = Query(1, ge=1, description="Page number"),
    page_size: int = Query(50, ge=1, le=200, description="Results per page"),
    team_id: Optional[int] = Query(None, description="Filter by team ID"),
    ...
    if sort_field not in _VALID_SORT_FIELDS:
        sort = "create_date desc"
    return await service.missing_salesperson_response(
        page=page,
        page_size=page_size,
        team_id=team_id,
        salesperson_id=salesperson_id,
...
@router.get(
    "/data-quality/missing-linked-contact",
    response_model=PaginatedMissingContactResponse,
    summary="Opportunities missing a linked contact / partner_id (paginated)",
)
@limiter.limit("30/minute")
async def crm_missing_linked_contact(
    request: Request,
    page: int = Query(1, ge=1, description="Page number"),
    page_size: int = Query(50, ge=1, le=200, description="Results per page"),
    team_id: Optional[int] = Query(None, description="Filter by team ID"),
    ...
    if sort_field not in _VALID_SORT_FIELDS:
        sort = "create_date desc"
    return await service.missing_linked_contact_response(
        page=page,
        page_size=page_size,
        team_id=team_id,
        salesperson_id=salesperson_id,
