...
**Last updated:** 2026-05-25 (LA_VERDE_DATA_STATE.md added as unified data-state reference)
...
|---|---|---|---|---|---|
| D-1 | English installment type names — **قيد التحقق البشري** | Cosmetic, non-urgent | Khaled (browser visual check) | 2026-05-22 | **كود مُنجز 2026-05-25 — بانتظار تحقق خالد في المتصفح.** Added `INSTALLMENT_TYPE_NAMES_EN` + `get_type_name_en()` to `installment_type_names.py` (13 IDs, names confirmed from Gate 1 discovery). Added `installment_type_name_en` field to `TypeBreakdownEntry` + `InstallmentRow` schemas. Backend (`_fetch_bucket_type_breakdown`, `_serialize_row`) now returns both `_ar` and `_en` fields. Frontend (`collections.js` `_buildBreakdownBars`, `drilldown.js` `_buildDrilldownBreakdownHtml` + `_makeInstallmentRow`) selects the name by `lang === 'ar'` — same pattern as project names. No numbers changed. 13 new tests in live paths (`test_kpi_service.py` D-1 section + `test_drilldowns.py` D-1 section). **تحقق مطلوب من خالد:** EN mode → English names (Regular, Garage, Down Payment…); AR mode → أسماء عربية لا تتغيّر. لا يُغلق البند حتى التأكيد. |
