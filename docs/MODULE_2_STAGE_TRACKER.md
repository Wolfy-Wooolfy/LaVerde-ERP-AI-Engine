...
**Last updated:** 2026-05-24 (D-2 RESOLVED — today_str() Cairo ZoneInfo fix, zero-delta verification)
...
| D-3 | Mobile responsive — Customer Accounts dashboard + M3-S7 drill-down panel (Module 3) | Non-urgent — requires deploy or LAN access | M3-S5 close + M3-S7 close (KhElmasry) | 2026-05-23 | صفحة `/customer-accounts/dashboard` اتأكّدت على الـ desktop (عربي + إنجليزي) في M3-S5. الـ M3-S7 drill-down panel اتأكّد desktop في 2026-05-23. الموبايل لسه ماتأكّدش للاتنين — التحقق محتاج deploy أو وصول من جهاز موبايل على نفس الشبكة. المطلوب وقت الاختبار: (1) الـ 3 KPI cards تتراص تحت بعض (grid-cols-1 sm:grid-cols-3)، (2) جدول KPI B مقروء (overflow-x-auto)، (3) قسم الاستردادات ظاهر صح، (4) مدخل "Customer Accounts" شغّال في الـ mobile drawer، (5) الـ drill-down panel ينزل full-screen على الموبايل (w-full على شاشات < lg) وينغلق صح. بعد التأكيد، يتحط tag على M3-S5 + M3-S7. المطلوب بعد الاختبار: تحديث MODULE_3_DRILLDOWN_PLAN.md §5 mobile note + تحديث هذا البند إلى "Done". |
