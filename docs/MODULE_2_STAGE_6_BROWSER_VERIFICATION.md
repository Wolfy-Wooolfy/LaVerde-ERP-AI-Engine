...
| 9.5 | **Body-portal RTL check** — panel DOM is now a direct `<body>` child (outside `<main>`); `dir=rtl` comes from `<html>` so `end-0` and slide direction still apply | Panel left-slides correctly; `end-0` places it at left edge | |
